...
| `03-workflow` | submitting starts a real `System.Workflow`, the user task lands in the supervising partner's inbox with its description and due date, and approve / return drive the workflow to its outcome. The escalation boundary timer is checked by arming (one queued task per workflow, dated +3 days) and then firing it — the queued task is pulled into the past so the engine's own poller runs it, rather than waiting three days |
