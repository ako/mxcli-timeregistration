# Mendix Project: TimeRegistration
...
```bash
./mxcli -p TimeRegistration.mpr    # Correct - uses local binary
```
...
```bash
./mxcli setup mxbuild -p TimeRegistration.mpr    # Auto-download for project's Mendix version
```

After setup, `mx` is at `~/.mxcli/mxbuild/{version}/modeler/mx`. Usage:

```bash
~/.mxcli/mxbuild/*/modeler/mx check TimeRegistration.mpr   # Validate project
./mxcli docker check -p TimeRegistration.mpr               # Alternative (auto-downloads mxbuild)
```
...
```bash
./mxcli -p TimeRegistration.mpr -c "SHOW MODULES"              # List all modules
./mxcli -p TimeRegistration.mpr -c "SHOW STRUCTURE"             # Project overview
./mxcli -p TimeRegistration.mpr -c "SHOW ENTITIES IN MyModule"  # Entities in a module
./mxcli -p TimeRegistration.mpr -c "DESCRIBE ENTITY MyModule.Customer"  # Entity details
```

### Execute an MDL Script File

```bash
./mxcli exec script.mdl -p TimeRegistration.mpr
```
...
```bash
./mxcli
# Then: CONNECT LOCAL 'TimeRegistration.mpr';
```
...
./mxcli check script.mdl -p TimeRegistration.mpr --references  # With reference validation
```
...
```bash
./mxcli check script.mdl -p TimeRegistration.mpr --references
```
...
# With colored output
./mxcli lint -p TimeRegistration.mpr --color

# List available rules
./mxcli lint -p TimeRegistration.mpr --list-rules

# Output as SARIF
./mxcli lint -p TimeRegistration.mpr --format sarif > results.sarif
```
...
./mxcli report -p TimeRegistration.mpr

# JSON report
./mxcli report -p TimeRegistration.mpr --format json

# HTML report
./mxcli report -p TimeRegistration.mpr --format html
```
